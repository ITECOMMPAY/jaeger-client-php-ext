...
bool TSocket::peek() {
  ...
  if (interruptListener_) {
    for (int retries = 0;;) {
      ...
      if (ret < 0) {
        ...
        throw Php::Exception("TTransportException::UNKNOWN: TTransportException: Unknown transport exception");
      } else if (ret > 0) {
  ...
  if (r == -1) {
    ...
    throw Php::Exception("TTransportException::UNKNOWN: recv()");
  }
  return (r > 0);
}
...
// Uses a low min RTO if asked to.
  ...
  if (!path_.empty()) {
    ...
    if (len > sizeof(((sockaddr_un*)NULL)->sun_path)) {
      ...
      throw Php::Exception("TTransportException::NOT_OPEN: Unix Domain socket path too long");
    }
...
void TSocket::local_open() {
  ...
  if (port_ < 0 || port_ > 0xFFFF) {
      throw Php::Exception("TTransportException::BAD_ARGS: Specified port is invalid");
  }
  ...
  if (error) {
    ...
    //GlobalOutput(errStr.c_str());
    close();
    throw Php::Exception("TTransportException::NOT_OPEN: Could not resolve host for client socket.");
  }
...
uint32_t TSocket::read(uint8_t* buf, uint32_t len) {
  if (socket_ == THRIFT_INVALID_SOCKET) {
      throw Php::Exception("TTransportException::NOT_OPEN: Called read on non-open socket");
  }
...
try_again:
  ...
  if (interruptListener_) {
    ...
    if (ret < 0) {
      ...
      throw Php::Exception("TTransportException::UNKNOWN: Unknown");
    } else if (ret > 0) {
      // Check the interruptListener
      if (fds[1].revents & THRIFT_POLLIN) {
          throw Php::Exception("TTransportException::INTERRUPTED: Interrupted");
      }
    } else /* ret == 0 */ {
        throw Php::Exception("TTransportException::TIMED_OUT: THRIFT_EAGAIN (timed out)");
    }
  ...
  if (got < 0) {
    if (errno_copy == THRIFT_EAGAIN) {
      ...
      if (recvTimeout_ == 0) {
          throw Php::Exception("TTransportException::TIMED_OUT: THRIFT_EAGAIN (unavailable resources)");
      }
    ...
    if (errno_copy == THRIFT_ENOTCONN) {
        throw Php::Exception("TTransportException::TIMED_OUT: THRIFT_ENOTCONN");
    }

    // Timed out!
    if (errno_copy == THRIFT_ETIMEDOUT) {
        throw Php::Exception("TTransportException::TIMED_OUT: THRIFT_ETIMEDOUT");
    }
...
void TSocket::write(const uint8_t* buf, uint32_t len) {
  ...
  while (sent < len) {
    ...
    if (b == 0) {
      ...
      // Raise an exception.
        throw Php::Exception("TTransportException::TIMED_OUT: send timeout expired");
    }
    sent += b;
  }
}
...
uint32_t TSocket::write_partial(const uint8_t* buf, uint32_t len) {
  if (socket_ == THRIFT_INVALID_SOCKET) {
      throw Php::Exception("TTransportException::TIMED_OUT: Called write on non-open socket");
  }
  ...
  if (b == 0) {
      throw Php::Exception("TTransportException::NOT_OPEN: Socket send returned 0.");
  }
  return b;
}
