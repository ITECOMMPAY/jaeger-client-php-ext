...
bool TFramedTransport::readFrame() {
    ...
    if (bytes_read == 0) {
      if (size_bytes_read == 0) {
        ...
        // EOF after a partial frame header.  Raise an exception.
          throw Php::Exception("TTransportException::END_OF_FILE: No more data to read after partial frame header.");
      }
    }
  ...
  if (sz < 0) {
      throw Php::Exception("TTransportException: (Invalid exception type): Frame size has negative value");
  }

  // Check for oversized frame
  if (sz > static_cast<int32_t>(maxFrameSize_))
      throw Php::Exception("TTransportException::CORRUPTED_DATA: Received an oversized frame");
...
void TFramedTransport::writeSlow(const uint8_t* buf, uint32_t len) {
  ...
  if (len + have < have /* overflow */ || len + have > 0x7fffffff) {
      throw Php::Exception("TTransportException::BAD_ARGS: Attempted to write over 2 GB to TFramedTransport.");
  }
...
void TMemoryBuffer::ensureCanWrite(uint32_t len) {
  ...
  while (len > avail) {
    ...
    if (new_size > maxBufferSize_) {
        throw Php::Exception("TTransportException::BAD_ARGS: Internal buffer size overflow");
    }
